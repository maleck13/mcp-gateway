...
func (m *mcpBrokerImpl) OnConfigChange(ctx context.Context, conf *config.MCPServersConfig) {
	...
	for _, mcpServer := range conf.Servers {
		...
		if _, ok := m.mcpServers[mcpServer.ID()]; !ok {
			...
			manager := upstream.NewUpstreamMCPManager(upstream.NewUpstreamMCP(mcpServer), m.listeningMCPServer, m.logger.With("sub-component", "mcp-manager"), m.managerTickerInterval, m.rebuildToolIndex)
