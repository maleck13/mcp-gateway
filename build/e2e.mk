...
test-e2e-run: test-e2e-deps ## Run e2e tests (assumes cluster is ready)
	...
	$(GINKGO) -v --tags=e2e --timeout=$(E2E_TIMEOUT) --procs=$(E2E_PROCS) ./tests/e2e
...
test-e2e-happy: test-e2e-deps ## Quick e2e test run for local development (no setup)
	...
	$(GINKGO) -v --tags=e2e --timeout=$(E2E_TIMEOUT) --procs=$(E2E_PROCS) $(E2E_GINKGO_FOCUS_HAPPY) ./tests/e2e
...
test-e2e-ci: test-e2e-deps ## Run PR-gate e2e tests in CI (excludes slow tier 2 suites, fail fast)
	$(GINKGO) -v --tags=e2e --timeout=$(E2E_TIMEOUT) --procs=$(E2E_PROCS) --fail-fast $(E2E_GINKGO_SKIP_TIER2) ./tests/e2e
...
test-e2e-auth-ci: test-e2e-deps ## Run auth e2e tests only (requires ci-auth-setup)
	$(GINKGO) -v --tags=e2e --timeout=$(E2E_TIMEOUT) --procs=$(E2E_PROCS) --fail-fast --focus="AuthPolicy" ./tests/e2e
...
test-e2e-https: test-e2e-deps ## Run HTTPS-focused E2E tests (requires cert-manager + MCP_PAT)
	...
	$(GINKGO) -v --tags=e2e --timeout=$(E2E_TIMEOUT) --procs=$(E2E_PROCS) --fail-fast --focus="\[HTTPS\]" ./tests/e2e
